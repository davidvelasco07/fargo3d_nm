...
void ReadVarFile(char *filename) {
  ...
  while (fgets(s, MAXLINELENGTH-1, input) != NULL ) {
    ...
    if (strncmp(s, "#GRIDINFO", 9) == 0) //Everything that is after
					 //this comment corresponds to
					 //nested mesh setup, and is
					 //ignored at this stage
      Inhibit = YES;
    if(name[0]!='#' && success == 1 && !Inhibit) {
...
#ifdef SPHERICAL
  if ((NX <= 2) && (*FRAME != 'F') && (*FRAME != 'f')) {
    mastererr ("It looks like you are running a 2D relaxation run,\n");
    mastererr ("but the 'Frame' parameter is not set to 'F' (fixed).\n");
    mastererr ("Unpredictable results may happen upon a stretched restart.\n");
  }
#endif
#endif
...
void ListVariablesIDL (char *filename)
{
  ...
  if (CPU_Rank == 0) {
    ...
    ////////////////////////////////////
    /* Below: added variable COORDPERMUT for compatibility with IDL scripts that reduce JUPITER's data */
#ifdef CARTESIAN
    fprintf(stream, "%s:'%s',$\n", "COORDPERMUT", "123");
    fprintf(stream, "%s:'%s',$\n", "COORDTYPE", "Cartesian");
#endif
