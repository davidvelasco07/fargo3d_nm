...
void AlgoGas(real *t) {
  ...
  while(dtemp<DT) { // DT LOOP    
    ...
    FARGO_SAFE(copy_velocities(VTEMP2V));
    FARGO_SAFE(FillGhosts(PrimitiveVariables())); //VERY IMPORTANT! FIRST CONTOUR AFTER ADVECTION.
    FARGO_SAFE(copy_velocities(V2VTEMP));
    ...
    *(Emfz->owner) = Emfz;
    ...
    FARGO_SAFE(thermal_diff_a());
    FARGO_SAFE(thermal_diff_b(dt));

    FARGO_SAFE(energy_release(dt));
