...
void SelectWriteMethod() {
  ...
  printf ("I do not output the ghost values\n");
...
void WriteDim () {
  ...
  if(CPU_Rank==0) {
    ...
    fprintf(dims,"%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t\n", \
	    "#XMIN", "XMAX", "YMIN", "YMAX", "ZMIN", "ZMAX",	\
	    "NX", "NY", "NZ", "CPUS_Y", "CPUS_Z", "NGHY", "NGHZ");		
    fprintf(dims,"%f\t%f\t%f\t%f\t%f\t%f\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n",	\
    	    XMIN, XMAX, YMIN, YMAX, ZMIN, ZMAX, NX, NY, NZ,	\
    	    Ncpu_x, Ncpu_y, NGHY, NGHZ);
    fclose(dims);
#ifdef LEGACY
...
void WriteField(Field *f, int n) {
  ...
  for (k=NGHZ; k<Nz+NGHZ; k++) { //Write grid without ghost cells
    for (j=NGHY; j<Ny+NGHY; j++) {
      fwrite(f->field_cpu+j*Nx+k*Stride, sizeof(real), Nx, fo);
    }
  }
  fclose(fo);
}
...
void WriteMerging(Field *f, int n) {
  ...
  for (k=0; k<NZ; k++) {
    for (j = 0; j<Ncpu_x; j++) {
      if ((J==j) && (k>=Z0) && (k<(Z0+Nz))) {
	fwrite(f->field_cpu+NGHY*Nx+(k-Z0)*Stride+NGHZ*Stride, sizeof(real)*Nx*Ny, 1, fo);
      }
      fflush(fo);
