...
void SetOverlapFlag () {
  ...
  while (g != NULL) {
	...
	r = r->next;
      ...
      ny=gncell[1];
      nz=gncell[2];
      if (g->Pitch_gpu == 1 && nx != 1)
	/* 2D setup without the X dimension: gncell[0] (nx) is the contiguous
	   (Y) extent and successive Z rows are spaced by Stride_gpu on the
	   device. The generic 'else' branch below would use Pitch_gpu (==1) as
	   the destination pitch, which is smaller than the row width and makes
	   cudaMemcpy2D fail with 'invalid pitch argument'. */
	cudaMemcpy2D (g->Hidden_d, g->Stride_gpu*sizeof(char), g->Hidden, nx*sizeof(char),nx*sizeof(char),ny,cudaMemcpyHostToDevice);
      else if (nx == 1)
