...
Force ComputeForce(real x, real y, real z,
...
#ifdef GPU
      FARGO_SAFE(_ComputeForce(x, y, z, rsmoothing, mass, item->Hidden_d));/* Function/Kernel Launcher. */
#else
  ...
  AdaptFieldsFromJ(current);/* Adapt everything back to the calling level (which should be finest level*/
