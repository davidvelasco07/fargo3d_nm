...
relevant dimensions. */
inline void BlockButcher(long *cube, DestBlocks *dblocks)
{
	long i, j, k;
	boolean cut;
	for (i = 0; i < 3; i++)
	...
	do
	{
		...
		for (i = 0; i < dblocks->nb; i++)
		{
			for (j = 0; j < NDIM; j++)
			{
				if (Periodic[j])
				{
					if ((dblocks->block[i][j] < 0) && (dblocks->block[i][j + 3] > 0))
					{				/* This block sits on a lower boundary */
					...
					else if ((dblocks->block[i][j] < ncorner_max0[j]) && (dblocks->block[i][j + 3] > ncorner_max0[j]))
					{
						...
						for (k = 0; k < 3; k++)
						{														   /* it therefore needs to be cut in two parts */
					...
					else if (dblocks->block[i][j] < 0)
					{											 /* the block does not need splitting but it needs to be shifted */
					...
					else if (dblocks->block[i][j + 3] > ncorner_max0[j])
					{ /* the block does not need splitting but it needs to be shifted */
						cut = TRUE;
...
inline boolean CubeIntersect(long *cube1, long *cube2, long *inter)
{
	long k;
...
inline boolean CubeIntersectPeriodic(long *cube1, long *cube2)
{
	long k[3], cube[6], le;
	long periodic[3];
	for (le = 0; le < 3; le++)
...
void BuildCommunicators()
{
	...
	while (destg != NULL)
	{
		srcg = Grid_CPU_list;
		while (srcg != NULL)
		{ /* The following test corresponds to
					...
					for (dim = 0; dim < NDIM; dim++)
					{
					   ...
					   of a time step sequence on that level*/
							   ...
							   communicators- */
										...
										if (ComListGhost != NULL)
											ComListGhost->prev = com;
										ComListGhost = com;
			...
			if (srcg->level == destg->level + 1)
			{ /* We look for MEAN communicators */
				for (i = 0; i < 3; i++)
				{
					cdest[i] = destg->gncorner_min[i];
					cdest[i + 3] = destg->gncorner_max[i];
