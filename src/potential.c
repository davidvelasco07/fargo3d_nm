...
void compute_potential(real dt) {
  ...
  if (Current_Level == LevMax){
    ...
    if (Corotating) {
      ...
      FARGO_for_all_patches(CorrectVtheta_noarg);
      OMEGAFRAME = OmegaNew;
    }
...
//<USER_DEFINED>
  ...
  //planetmass_taper = (PhysicalTime >= MASSTAPER ? 1.0 : sin(M_PI*.5*PhysicalTime/MASSTAPER));
//<\USER_DEFINED>
...
//<EXTERNAL>
  ...
  real rescaleradius = RESCALERADIUS;
...
//<#>
	...
	for(n=0; n<nb; n++) {
	  ...
	  dist = ((XC-rescaleradius*xplanet[n])*(XC-rescaleradius*xplanet[n])+
		  (YC-rescaleradius*yplanet[n])*(YC-rescaleradius*yplanet[n])+
		  (ZC-rescaleradius*zplanet[n])*(ZC-rescaleradius*zplanet[n]));
#ifndef NODEFAULTSTAR
