...
// |-----+-----+-----+-----|
  ...
  InvDiffXmed = (real *)malloc(sizeof(real)*(Nx+1+2*NGHX)); //+1 because we use element #1
  InvDiffYmed = (real *)malloc(sizeof(real)*(Ny+1+2*NGHY)); //even if X, Y or Z is 'undefined'
  InvDiffZmed = (real *)malloc(sizeof(real)*(Nz+1+2*NGHZ));
