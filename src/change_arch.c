...
void ChangeArch() {
  ...
  if(func_arch == NULL) {
    masterprint("Error!! %s cannot be opened.\n", FUNCARCHFILE);
  }
  ...
  LumPointSrc  = LumPointSrc_cpu;
  LumSphericalSrc  = LumSphericalSrc_cpu;
  LumExponentialSrc  = LumExponentialSrc_cpu;
  LumPoissonSrc  = LumPoissonSrc_cpu;
  DivideByRho = DivideByRho_cpu;
  MultiplyByRho = MultiplyByRho_cpu;
  ...
  Update_a_X    = Update_a_X_cpu;
  Update_b_X    = Update_b_X_cpu;
  Update_a_Y    = Update_a_Y_cpu;
  Update_b_Y    = Update_b_Y_cpu;
  Update_a_Z    = Update_a_Z_cpu;
  Update_b_Z    = Update_b_Z_cpu;
  ...
  Update_dens_a_X    = Update_dens_a_X_cpu;
  Update_dens_a_Y    = Update_dens_a_Y_cpu;
  Update_dens_a_Z    = Update_dens_a_Z_cpu;
  ...
  boundary_xmin  = boundary_xmin_cpu;
  boundary_xmax  = boundary_xmax_cpu;
  ...
  Fill_GhostsX_erad =  Fill_GhostsX_erad_cpu;
  ...
  ComputeTemperature  =             ComputeTemperature_cpu;            
  ComputeOpacity  = 		    ComputeOpacity_cpu;		    
  Heating  = 		            Heating_cpu; 		    
  ComputeEtas  =		    ComputeEtas_cpu;		    
  ComputeDiffusionCoefficients  =   ComputeDiffusionCoefficients_cpu;   
  ComputeMatrixElements  =          ComputeMatrixElements_cpu;   
  SetRTBoundaryConditions  = 	    SetRTBoundaryConditions_cpu; 	    
  RadEnergyToTemperature  = 	    RadEnergyToTemperature_cpu; 	    
  TemperatureToEnergy  = 	    TemperatureToEnergy_cpu; 	     
  MatrixIterate  =                  MatrixIterate_cpu;  
  NormRHS  =                        NormRHS_cpu;
  PredictNewEnergyRad =             PredictNewEnergyRad_cpu;
  NewDiffEnergyRad =                NewDiffEnergyRad_cpu;

  //--- STELLAR IRRADIATION FUNCTIONS -------------------

#ifdef IRRADIATION
  InProcessRadialScan  = InProcessRadialScan_cpu;   
  SetBackgroundSlice   = SetBackgroundSlice_cpu;     
  GetBackgroundSlice   = GetBackgroundSlice_cpu;     
  InterCPUScan	       = InterCPUScan_cpu;	      
  OffsetPatches	       = OffsetPatches_cpu;	      
  RT_IRR_TauStellar    = RT_IRR_TauStellar_cpu;     
  ComputeStellarHeating = ComputeStellarHeating_cpu;
#endif

  _PlanetHeating = _PlanetHeating_cpu;
  ReMap = ReMap_cpu;
  ReMapCopy = ReMapCopy_cpu;
  ...
  while (fgets(s, MAXLINELENGTH-1, func_arch) != NULL) {
	...
	if(strval[0] == 'g'){
	  ...
	  LumPointSrc  = LumPointSrc_gpu;
	  LumSphericalSrc  = LumSphericalSrc_gpu;
	  LumExponentialSrc  = LumExponentialSrc_gpu;
	  LumPoissonSrc  = LumPoissonSrc_gpu;
	...
	if(strval[0] == 'g'){
	  ...
	  MultiplyByRho = MultiplyByRho_gpu;
...
#ifdef MPICUDA
	  comm = comm_gpu;
	  masterprint("Communications are done directy on GPU\n");
#else
	  masterprint("Warning: your version of MPI does not allow direct GPU-GPU communications.\n");
	  masterprint("Communications are done through the hosts (CPUs), and may not be very efficient.\n");
#endif
	}
      }
      if (strcmp(name, "boundaries") == 0) {
	if(strval[0] == 'g'){
	  boundary_xmin  = boundary_xmin_gpu;
	  boundary_xmax  = boundary_xmax_gpu;
	...
	if(strval[0] == 'g'){
          ...
          Fill_GhostsX_erad =  Fill_GhostsX_erad_gpu;
	...
	if(strval[0] == 'g'){
      ...
      if (strcmp(name, "rt") == 0) {
	if(strval[0] == 'g'){
	  ComputeTemperature  =             ComputeTemperature_gpu;            
	  ComputeOpacity  = 		    ComputeOpacity_gpu;		    
	  Heating  = 		            Heating_gpu; 		    
	  ComputeEtas  =		    ComputeEtas_gpu;		    
	  ComputeDiffusionCoefficients  =   ComputeDiffusionCoefficients_gpu;   
	  ComputeMatrixElements  =          ComputeMatrixElements_gpu;   
	  SetRTBoundaryConditions  = 	    SetRTBoundaryConditions_gpu; 	    
	  RadEnergyToTemperature  = 	    RadEnergyToTemperature_gpu; 	    
	  TemperatureToEnergy  = 	    TemperatureToEnergy_gpu; 	     
	  MatrixIterate  =                  MatrixIterate_gpu;  
	  NormRHS =                         NormRHS_gpu;
	  PredictNewEnergyRad =             PredictNewEnergyRad_gpu;
	  NewDiffEnergyRad =                NewDiffEnergyRad_gpu;
	  //printf("Radiative transfer runs on the GPU\n");
	}
      }

#ifdef IRRADIATION
      if (strcmp(name, "irradiation") == 0) {
	if(strval[0] == 'g'){
	  InProcessRadialScan   = InProcessRadialScan_gpu;
	  SetBackgroundSlice    = SetBackgroundSlice_gpu;     
	  GetBackgroundSlice    = GetBackgroundSlice_gpu;     
	  //#ifndef MPICUDA
	  InterCPUScan	        = InterCPUScan_cpu;	      
	  //#else
	  //	  InterCPUScan	        = InterCPUScan_gpu;	 //MPI_Scan not yet CUDA aware
	  //#endif
	  OffsetPatches	        = OffsetPatches_gpu;	      
	  RT_IRR_TauStellar     = RT_IRR_TauStellar_gpu;
	  ComputeStellarHeating = ComputeStellarHeating_gpu;
	  //printf("Stellar irradiation runs on the GPU\n");
	}
      }
#endif

      if (strcmp(name, "planetheating") == 0) {
	if(strval[0] == 'g'){
	  _PlanetHeating  = _PlanetHeating_gpu;
	...
	if(strval[0] == 'g'){
	  ReMap = ReMap_gpu;
	  ReMapCopy = ReMapCopy_gpu;
	  //printf("ReMap runs on the GPU\n");
	}
      }
