...
void MakeDir(char *string)
{
  ...
  if (CPU_Rank)
    MPI_Recv(&foo, 1, MPI_INT, CPU_Rank - 1, 53, DomainComm, &fargostat);
  dir = opendir(string);
  if (dir)
  {
    closedir(dir);
  ...
  if (CPU_Rank < CPU_Number - 1)
    MPI_Send(&foo, 1, MPI_INT, CPU_Rank + 1, 53, DomainComm);
}
...
void InitSurfaces()
{
...
#if defined(X)
    Syj(j) = Ymin(j) * Dx;
    Szj(j) = 0.5 * (Ymin(j + 1) * Ymin(j + 1) - Ymin(j) * Ymin(j)) * Dx;
#else
    Syj(j) = Ymin(j);
  ...
  for (j = 0; j < Ny + 2 * NGHY; j++)
  {
    Sxj(j) = 0.5 * (Ymin(j + 1) * Ymin(j + 1) - Ymin(j) * Ymin(j));
#ifdef X
...
#else
    Syj(j) = Ymin(j) * Ymin(j);
    Szj(j) = 0.5 * (Ymin(j + 1) * Ymin(j + 1) - Ymin(j) * Ymin(j));
    InvVj(j) = 3. / ((Ymin(j + 1) * Ymin(j + 1) * Ymin(j + 1) -
                      Ymin(j) * Ymin(j) * Ymin(j)));
#endif
  }
