 ...
 while (fluid != NULL) {
   for (i = 0; i < nvar; i++) {
     ...
     if ( field == _Density_ ){
       ...
       JSOUTPUT(fluid->EnergyRad);
     }
     else if ( field == _QP_ ){
       JSINPUT(fluid->Qplus);
       JSOUTPUT(fluid->Qplus);
     }
#endif
#ifdef LABELED
     else if ( field == _Label_ ){
       JSINPUT(fluid->Label);
       JSOUTPUT(fluid->Label);
     }
#endif
...
void ExecComm (levsrc, levdest, type, nvar, fieldtype) 
     long levsrc, levdest, type, nvar;
