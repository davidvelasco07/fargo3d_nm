...
void ExecCommDownMeanVar (long lev, long nvar, int *fieldtype){
  ...
  while (com != NULL) {
	...
	for (h = 0; h < 3; h++) {
	  ...
	  stridec[h] = (h > 0 ? stridec[h-1]*\
			(imaxd[h-1]-imind[h-1]+1) : 1);
	  stridecr[h] = (h > 0 ? stridecr[h-1]*\
			 (imaxd[h-1]-imind[h-1]) : 1);
	...
	for (le=0; le<nvar*NbFluids; le++) {
	  for (k=0; k < (imaxd[2] - imind[2]); k += 1) {
		...
		/*	for (iid[2] = centered[le][2]-1; iid[2] <= 1; iid[2]++) { 
                  coef2 =  (1+(1-centered[le][2])*iid[2]*iid[2]);
		  for (iid[1] = centered[le][1]-1; iid[1] <= 1; iid[1]++) { 
                    coef1 = coef2 * (1+(1-centered[le][1])*iid[1]*iid[1]);
	            for (iid[0] = centered[le][0]-1; iid[0] <= 1; iid[0]++) { 
                      coef0 = coef1 * (1+(1-centered[le][0])*iid[0]*iid[0]);
		      src = source[le][(is[0]+iid[0])*strides[0] + (is[1]+iid[1])*strides[1] + (is[2]+iid[2])*strides[2]];
