...
ScalarField *CreateScalarField2D (desc, name, ptr)
     tGrid_CPU *desc;
     char *name;
     real *ptr;
{
...
FluidPatch *CreateFluidPatch (desc, name)
     tGrid_CPU *desc;
     char *name;
{
  char *Name;
  ...
  ScalarField *Density, *Energy, *TotalEnergy, *Potential, *OpticalDepth, *EnergyRad, *Qplus, *Label;
  ScalarField *StellarHeating, *EradDeriv;
  VectorField *Velocity,*MomentaP,*MomentaM;
  FluidPatch *patch;
  void *ptr_gpu;
  size_t pitch;
  ...
  jump = (3+NDIM);
#ifdef RADIATIVE_TRANSFER
  EnergyRad = CreateScalarField (desc, "energyrad", StartField+(jump++)*Size);
  Qplus = CreateScalarField (desc, "qplus", StartField+(jump++)*Size);
#endif
#ifdef LABELED
  Label = CreateScalarField (desc, "label", StartField+jump*Size);
#endif
 ...
 patch->Ptr[_Vz_] = Velocity->Field[_Z_];
  ...
  patch->Ptr[_QP_]   = Qplus->Field;
#endif
#ifdef LABELED
  patch->Ptr[_Label_] = Label->Field;
#endif
  ...
  patch->Ptr_gpu[_Potential_] = make_cudaPitchedPtr(Potential->Field_gpu,Density->pitch,size[0],size[1]);
#ifdef X
...
#endif
#ifdef RADIATIVE_TRANSFER
  patch->Ptr_gpu[_Erad_] = make_cudaPitchedPtr(EnergyRad->Field_gpu,Density->pitch,size[0],size[1]);
  patch->Ptr_gpu[_Qp_] = make_cudaPitchedPtr(Qplus->Field_gpu,Density->pitch,size[0],size[1]);
#endif 
#ifdef LABELED
  patch->Ptr_gpu[_Label_] = make_cudaPitchedPtr(Label->Field_gpu,Density->pitch,size[0],size[1]);
#endif
  ...
  nvar++;
#endif
  for (dim = 0; dim < 3; dim++){ // 3, not NDIM
    dimp1 = (dim == 0);
    dimp2 = 2 - (dim == 2);
#ifndef GPU
