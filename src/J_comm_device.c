...
void JCommunicatorGPU (jCommunicator *comm)
{
  ...
  extent = make_cudaExtent ((xmaxdest-xmindest)*sizeof(real),(ymaxdest-ymindest),(zmaxdest-zmindest)*nvar*NbFluids);
...
void FillCommArrays(){
 ...
 while (grid != NULL){
   if (grid->cpu == CPU_Rank){
     ...
     cudaMalloc((void**)&grid->centered_gpu,nvar*NbFluids*3*sizeof(int));
     cudaMalloc((void**)&grid->source,nvar*NbFluids*sizeof(real));
     cudaMalloc((void**)&grid->dest,  nvar*NbFluids*sizeof(real));
...
#ifdef FLOAT
	MPI_Isend (comm->bufferGPU.ptr, comm->yzsize*nvar*NbFluids*comm->bufferGPU.pitch/sizeof(real), \
		   MPI_FLOAT, comm->CPU_dest, comm->tag,		\
		   MPI_COMM_WORLD, Req+nbreq++);
#else
	MPI_Isend (comm->bufferGPU.ptr, comm->yzsize*nvar*NbFluids*comm->bufferGPU.pitch/sizeof(real), \
		   MPI_DOUBLE, comm->CPU_dest, comm->tag,		\
		     MPI_COMM_WORLD, Req+nbreq++);
#endif
      }
...
#ifdef FLOAT
	MPI_Irecv (comm->bufferGPU.ptr, comm->yzsize*nvar*NbFluids*comm->bufferGPU.pitch/sizeof(real), \
		   MPI_FLOAT, comm->CPU_src, comm->tag,		\
		   MPI_COMM_WORLD, Req+nbreq++);
#else
	MPI_Irecv (comm->bufferGPU.ptr, comm->yzsize*nvar*NbFluids*comm->bufferGPU.pitch/sizeof(real), \
		   MPI_DOUBLE, comm->CPU_src, comm->tag,		\
		   MPI_COMM_WORLD, Req+nbreq++);
#endif
...
#endif
#ifdef RADIATIVE_TRANSFER
     else if ( field == _Erad_ ){
       JSInput_GPU(fluid->EnergyRad, __LINE__, __FILE__);
     }
     else if ( field == _QP_ ){
       JSInput_GPU(fluid->Qplus, __LINE__, __FILE__);
     }
#endif
#ifdef LABELED
     else if ( field == _Label_ ){
       JSInput_GPU(fluid->Label, __LINE__, __FILE__);
     }
#endif
 ...
 while (fluid != NULL) {
   for (i = 0; i < nvar; i++) {
     ...
     if ( field == _Density_ ){
       ...
       JSInput_GPU(fluid->EnergyRad, __LINE__, __FILE__);
       JSOutput_GPU(fluid->EnergyRad, __LINE__, __FILE__);
     }
     else if ( field == _QP_ ){
       JSInput_GPU(fluid->Qplus, __LINE__, __FILE__);
       JSOutput_GPU(fluid->EnergyRad, __LINE__, __FILE__);
       ...
       JSInput_GPU(fluid->Label, __LINE__, __FILE__);
       JSOutput_GPU(fluid->Label, __LINE__, __FILE__);
     }
#endif
