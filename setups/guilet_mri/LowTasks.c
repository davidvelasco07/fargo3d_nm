...
void Check_CUDA_Blocks_Consistency () {
  ...
  if (problem == YES) {
    ...
    mastererr ("You must edit the opt file of your SETUP in the setups/SETUP/ directory\n");
...
void CreateFields() {
  ...
  *(Emfz->owner) = Emfz;
  ...
  DiffEnergyrad = CreateField ("differad",0,0,0,0);
  QPlus = CreateField ("qplus",0,0,0,0);

  ///All other fields than those above are aliases in a 3D setup

#ifdef X
  OpaPlanck   = CREATEFIELDALIAS ("opap",Mpx,0);
  Temperature = CREATEFIELDALIAS ("temperature",Mmx,0);
  RHS         = CREATEFIELDALIAS ("rhs",Vx_temp,0);
#else
  OpaPlanck   = CreateField ("opap",0,0,0,0);
  Temperature = CreateField ("temperature",0,0,0,0);
  RHS         = CreateField ("rhs",0,0,0,0);
#endif

#ifdef Y
  DiffCoef = CREATEFIELDALIAS ("diffcoef",Mpy,0);
  BB       = CREATEFIELDALIAS ("bb",Mmy,0);
  Resid    = CREATEFIELDALIAS ("resid",Vy_temp,0);
#else
  DiffCoef = CreateField ("diffcoef",0,0,0,0);
  BB       = CreateField ("bb",0,0,0,0);
  Resid    = CreateField ("resid",0,0,0,0);
#endif

#ifdef Z
  Eta1       = CREATEFIELDALIAS ("eta1",Mpz,0);
  Eta2       = CREATEFIELDALIAS ("eta2",Mmz,0);
  StellarRad = CREATEFIELDALIAS ("srad",Vz_temp,0);
#else
  Eta1       = CreateField ("eta1",0,0,0,0);
  Eta2       = CreateField ("eta2",0,0,0,0);
  StellarRad = CreateField ("srad",0,0,0,0);
#endif

  LeftX = CREATEFIELDALIAS ("leftx",Slope,0);
  LeftY = CreateField ("lefty",0,0,0,0); //DivRho cannot be aliased (used as workarray)
  LeftZ = CREATEFIELDALIAS ("leftz",DensStar,0);
  RightX = CREATEFIELDALIAS ("rightx",Qs,0);
  RightY = CREATEFIELDALIAS ("righty",Pressure,0);
  RightZ = CREATEFIELDALIAS ("rightz",QR,0);
#endif
#ifdef IRRADIATION
  LocalDepth = CreateField2D ("PEdepth", ZX);
...
real ComputeMass() {
  ...
  if(CPU_Master) printf("TotalMass = %3.10f \n", totalmass );
#else
