...
void CondInit() {
  ...
  for (k=0; k<Nz+2*NGHZ; k++) {
    for (j=0; j<Ny+2*NGHY; j++) {
      ...
      aspectratio = pow(9.0/32.0*SIGMA0*SIGMA0*KAPPA*NU*pow(R_MU,4)/STEFANK/pow(G*MSTAR,3)*R0,0.125)*pow(Ymed(j)/R0,(1.0-2*SIGMASLOPE)/8.0);

      //      aspectratio = ASPECTRATIO * pow(Ymed(j)/R0, FLARINGINDEX);
