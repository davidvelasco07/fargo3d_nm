...
#ifdef POTENTIAL_DIRECT
	gradpot = (ymin(j)-YBODY)/pow((xmed(i)-XBODY)*(xmed(i)-XBODY)+\
				      (ymin(j)-YBODY)*(ymin(j)-YBODY)+	\
				      (zmed(k)-ZBODY)*(zmed(k)-ZBODY),1.5);
	vy_temp[ll] -= G*MASSBODY*gradpot*dt;
#endif // ENDIF POTENTIAL_DIRECT
#endif //ENDIF Y
