...
extern "C" int Host2Dev3D(Field *field) {
 int status,di;
 tGrid_CPU *grid;
 grid = field->desc;
 di=0;
 ...
 if (Nx +2*NGHX== 1)
   status = cudaMemcpy2D (field->field_gpu, grid->Stride_gpu*sizeof(real), field->field_cpu, (Ny+2*NGHY)*sizeof(real),(Ny+2*NGHY)*sizeof(real),Nz+2*NGHZ,cudaMemcpyHostToDevice);
 else
    status = cudaMemcpy2D (field->field_gpu, grid->Pitch_gpu*sizeof(real), field->field_cpu, (Nx+2*NGHX)*sizeof(real),(Nx+2*NGHX)*sizeof(real),(Ny+2*NGHY)*(Nz+2*NGHZ),cudaMemcpyHostToDevice);
...
extern "C" int Dev2Host3D(Field *field) {
 int status,di;
 di=0;
tGrid_CPU *grid;
 grid = field->desc;
...
#endif
  if (Nx == 1)
    status = cudaMemcpy2D (field->field_cpu, Ny*sizeof(real), field->field_gpu, grid->Stride_gpu*sizeof(real),\
			 Ny*sizeof(real),Nz,cudaMemcpyDeviceToHost);
  else
    status = cudaMemcpy2D (field->field_cpu, Nx*sizeof(real), field->field_gpu, grid->Pitch_gpu*sizeof(real), \
...
extern "C" int Host2Dev2D (Field2D *F) {
 ...
 int Nz = 1;
#endif
  if (F->kind == YZ) {
    err = cudaMemcpy2D (F->field_gpu, (grid->Pitch2D)*sizeof(real),\
			F->field_cpu, (Ny+2*NGHY)*sizeof(real),		\
			(Ny+2*NGHY)*sizeof(real),Nz+2*NGHZ,\
  ...
  if (F->kind == ZX) {
    return (int)cudaMemcpy2D (F->field_gpu, (grid->Pitch2D)*sizeof(real),\
			      F->field_cpu, (Nx+2*NGHX)*sizeof(real),	\
			      (Nx+2*NGHX)*sizeof(real), Nz+2*NGHZ, \
			      cudaMemcpyHostToDevice);
  }
  return 1; //By default raises an error since comm for 'kind' not implemented
}

extern "C" int Dev2Host2D (Field2D *F) {
  if (F->kind == YZ)
    return (int)cudaMemcpy2D ((F->field_cpu), ((Ny+2*NGHY)*sizeof(real)), (F->field_gpu), (F->desc->Pitch2D*sizeof(real)), \
			      ((Ny+2*NGHY)*sizeof(real)),(Nz+2*NGHZ),cudaMemcpyDeviceToHost);
  else if (F->kind == ZX)
    return (int)cudaMemcpy2D (F->field_cpu, (Nx+2*NGHX)*sizeof(real), F->field_gpu, (F->desc->Pitch2D)*sizeof(real), \
