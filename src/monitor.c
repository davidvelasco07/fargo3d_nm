...
void InitMonitoring() {
  ...
  InitFunctionMonitoring (FORCE,   mon_force, "force", TOTAL, "YCZC", INDEP_PLANET);
  InitFunctionMonitoring (TORQ,    mon_torq, "torq", TOTAL, "YCZC", DEP_PLANET);
  InitFunctionMonitoring (TORQEXCL,mon_torq_excl, "torqexcl", TOTAL, "YCZC", DEP_PLANET);
...
void AllLevelSum (int idx, int r) {
  ...
  for (i = 0; i < nb; i++) {
    ...
    if (mon_pldp[idx] == INDEP_PLANET)
      sprintf (filename, "%s/%s_all_levels.dat", OUTPUTDIR, mon_name[idx]);
    else
